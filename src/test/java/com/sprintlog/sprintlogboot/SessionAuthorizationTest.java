...
class SessionAuthorizationTest {
    ...
    /** 로그인 세션 + CSRF 토큰을 실어 POST /api/v1/users/role 로 권한 변경 요청(미인증이면 sessionCookie=null). */
    private ResponseEntity<String> putRole(RestTemplate rt, String sessionCookie, String targetEmail, Role role) {
        ...
        return rt.exchange(base + "/api/v1/users/role", HttpMethod.POST, new HttpEntity<>(body, headers), String.class);
    }
