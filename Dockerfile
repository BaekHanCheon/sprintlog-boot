...
# Docker는 빌드할 때마다 레이어 캐시를 만듭니다. 이전 빌드와 파일 내용이 바뀌지 않았다면 이전에 저장한 레이어 캐시를 그대로 재사용함
...
# CMD, RUN 은 기본 실행 명령을 의미, 컨테이너 실행 시에 다른 명령어가 주어지면 그 명령어로 대체됨.
